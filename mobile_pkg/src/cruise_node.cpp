...
void load_qd_template() {
    ...
    vector<string> template_dirs = {
        pkg_path + "/data",     // data文件夹（敌方目标模板）
        pkg_path + "/muban"     // muban文件夹（友方目标模板）
    };

    vector<string> image_files;
    
    // 搜索所有模板目录中的图片文件
    for (const auto& template_dir : template_dirs) {
        DIR* dir = opendir(template_dir.c_str());
        if (!dir) {
            ROS_WARN("[模板加载] 无法打开模板目录: %s", template_dir.c_str());
            continue;
        }
        ...
        while ((entry = readdir(dir)) != nullptr) {
            if (entry->d_type == DT_REG) {
                ...
                if (ext.find(".jpg") != string::npos ||
                    ...
                    image_files.push_back(template_dir + "/" + filename);
                }
            }
        }
        closedir(dir);
    ...
    if (image_files.empty()) {
        ROS_ERROR("[模板加载] 所有模板目录中都没有找到任何图片文件！");
        return;
    }
    ...
    for (const auto& path : image_files) {
        ...
        if ((lower_path.find("qd") != string::npos || 
            ...
            lower_path.find("data") != string::npos) {
            qd_path = path;
            break;
        }
    }
    
    // 如果data文件夹中没有找到，搜索muban文件夹中的QD模板
    if (qd_path.empty()) {
        for (const auto& path : image_files) {
            string lower_path = path;
            transform(lower_path.begin(), lower_path.end(), lower_path.begin(), ::tolower);
...
void load_character_templates() {
    ...
    bool friendly_loaded = false;

    for (const auto& template_dir : template_dirs) {
        DIR* dir = opendir(template_dir.c_str());
        if (!dir) {
            ROS_WARN("[角色模板] 无法打开模板目录: %s", template_dir.c_str());
            continue;
        }
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            string full_path = template_dir + "/" + filename;
            ...
            // 优先根据文件名识别模板类型
            if (lower_name.find("qd") != string::npos || 
                lower_name.find("enemy") != string::npos ||
            ...
            else if (lower_name.find("xn") != string::npos ||
                     lower_name.find("friendly") != string::npos ||
                ...
                // 如果文件名不明确，根据文件夹路径判断
                if (template_dir.find("data") != string::npos && !enemy_loaded) {
                    enemy_target_color_avg = avg_color;
                    has_enemy_target_template = true;
                    enemy_loaded = true;
                    ROS_INFO("[角色模板] 从data文件夹加载敌方目标模板: %s, BGR=(%.1f, %.1f, %.1f)",
                             filename.c_str(), avg_color[0], avg_color[1], avg_color[2]);
                }
                else if (template_dir.find("muban") != string::npos && !friendly_loaded) {
                    friendly_target_color_avg = avg_color;
                    has_friendly_target_template = true;
                    friendly_loaded = true;
                    ROS_INFO("[角色模板] 从muban文件夹加载友方目标模板: %s, BGR=(%.1f, %.1f, %.1f)",
                             filename.c_str(), avg_color[0], avg_color[1], avg_color[2]);
                }
            }
        }
...
// 新增：中轴线分段检测函数 - 优化模板匹配，将人物中轴线分为上中下三段
vector<string> analyze_person_centerline_segments(const Mat& person_roi) {
    ...
    if (top_segment.width > 0 && top_segment.height > 0) {
        ...
        // 头部区域亮度检测（敌方模板通常头部较暗）
        double head_brightness = (avg_color[0] + avg_color[1] + avg_color[2]) / 3.0;
        if (head_brightness < 50 && segment_colors[0] == "unknown") {
            segment_colors[0] = "enemy_head"; // 敌方头部特征
        }
    }
    
    // 分析中段（躯干区域）- 重点检测服装颜色特征
    ...
    if (middle_segment.width > 0 && middle_segment.height > 0) {
        ...
        // 躯干区域亮度检测（敌方模板通常躯干较暗，友方较亮）
        double torso_brightness = (avg_color[0] + avg_color[1] + avg_color[2]) / 3.0;
        if (torso_brightness < 60 && segment_colors[1] == "unknown") {
            segment_colors[1] = "enemy_torso"; // 敌方躯干特征
        } else if (torso_brightness > 150 && segment_colors[1] == "unknown") {
            segment_colors[1] = "friendly_torso"; // 友方躯干特征
        }
    }
    
    // 分析下段（腿部区域）- 重点检测敌方模板特征
    ...
    if (bottom_segment.width > 0 && bottom_segment.height > 0) {
        ...
        // 腿部区域亮度检测（敌方模板通常腿部较暗）
        double leg_brightness = (avg_color[0] + avg_color[1] + avg_color[2]) / 3.0;
        if (leg_brightness < 50 && segment_colors[2] == "unknown") {
            segment_colors[2] = "enemy_leg"; // 敌方腿部特征
        }
    }
    
    // 输出分段检测结果
    ROS_INFO("[中轴线分段检测] 上段(头部): %s, 中段(躯干): %s, 下段(腿部): %s", 
             segment_colors[0].c_str(), segment_colors[1].c_str(), segment_colors[2].c_str());
...
string analyze_person_clothing_color(const Mat& person_roi) {
    ...
    // 基于模板图片颜色特征的智能匹配
    string template_based_color = "unknown";
    
    // 敌方模板颜色特征（data/QD.jpg）：深色系为主
    vector<string> enemy_template_colors = {"black", "dark_gray", "dark_color", "brown", "gray"};
    
    // 友方模板颜色特征（muban/xn.jpg）：浅色系为主
    vector<string> friendly_template_colors = {"white", "light_gray", "light_color", "pink", "light_blue", "yellow"};
    
    // 计算与敌方模板的颜色匹配度
    double enemy_match_score = 0.0;
    for (const auto& enemy_color : enemy_template_colors) {
        if (color_distribution.find(enemy_color) != color_distribution.end()) {
            enemy_match_score += color_distribution[enemy_color];
        }
    }
    
    // 计算与友方模板的颜色匹配度
    double friendly_match_score = 0.0;
    for (const auto& friendly_color : friendly_template_colors) {
        if (color_distribution.find(friendly_color) != color_distribution.end()) {
            friendly_match_score += color_distribution[friendly_color];
        }
    }
    ...
    for (const auto& pair : color_distribution) {
        if (pair.first != "unknown") {
            total_pixels += pair.second;
        }
    }
    
    // 计算匹配度百分比
    double enemy_match_percent = (total_pixels > 0) ? (enemy_match_score / total_pixels) : 0.0;
    double friendly_match_percent = (total_pixels > 0) ? (friendly_match_score / total_pixels) : 0.0;
    
    // 基于匹配度进行判断
    if (enemy_match_percent > friendly_match_percent && enemy_match_percent > 0.3) {
        template_based_color = "enemy_like";
    } else if (friendly_match_percent > enemy_match_percent && friendly_match_percent > 0.3) {
        template_based_color = "friendly_like";
    }
    ...
    ROS_INFO("[颜色匹配度] 敌方模板匹配度: %.2f%%", enemy_match_percent * 100);
    ROS_INFO("[颜色匹配度] 友方模板匹配度: %.2f%%", friendly_match_percent * 100);
    ...
    // 返回基于模板匹配的颜色判断
    if (!template_based_color.empty()) {
        ROS_INFO("[模板匹配] 基于模板的颜色判断: %s", template_based_color.c_str());
        return template_based_color;
    }
...
double improved_template_match(const Mat& img, const Mat& template_img, const string& clothing_color) {
    ...
    // 友方特征颜色：降低匹配得分权重
    else if (clothing_color == "pink" || clothing_color == "pinkish" || clothing_color == "white" || 
             clothing_color == "light_gray" || clothing_color == "light_blue") {
        ...
        if (!contours.empty()) {
            ...
            if (max_idx >= 0) {
                ...
                if (red_box.width >= 50 && red_box.height >= 50 && 
                    ...
                    // 直接使用整个红色方框区域，不再进行向内裁剪
                    Rect digit_roi = red_box;
                    
                    // 确保区域在图像范围内
                    digit_roi = digit_roi & Rect(0, 0, img.cols, img.rows);
                    
                    if (digit_roi.width >= 40 && digit_roi.height >= 40) {
                        ROS_INFO("[Digit Region Extraction] Detected red box, using full red box region: %dx%d", 
                                digit_roi.width, digit_roi.height);
                        return digit_roi;
                    }
                }
            }
...
bool detect_number_by_contour(const Mat& img, int& detected_num, double& contour_score) {
    ...
    // 根据新的宽高比判断逻辑：宽度是否超过高度的一半
    bool is_width_over_half_height = num_rect.width > (num_rect.height / 2);
    ROS_INFO("[轮廓识别] 数字轮廓宽度: %d, 高度: %d, 宽度是否超过高度一半: %s", 
             num_rect.width, num_rect.height, is_width_over_half_height ? "是" : "否");
    
    // 计算轮廓识别得分（基于轮廓面积和宽高比判断的置信度）
    double area_confidence = min(1.0, (double)num_rect.area() / 1000.0); // 面积越大越可信
    double ratio_confidence = 0.8; // 新的宽高比判断逻辑固定置信度为0.8
    contour_score = (area_confidence + ratio_confidence) / 2.0;
    
    if (!is_width_over_half_height) {
        detected_num = 1;
...
void detect_num(const Mat& img) {
    ...
    if (template_scores.size() >= 2) {
        // 找到得分最高的两个模板
        double max_score = template_best_score;
        double second_score = 0.0;
        
        for (double score : template_scores) {
            if (score < max_score && score > second_score) {
                second_score = score;
            }
        }
        
        score_gap = max_score - second_score;
        ROS_INFO("[模板匹配] 得分差距: %.3f (最高分: %.3f, 次高分: %.3f)", 
                 score_gap, max_score, second_score);
    }
    
    // 只使用模板匹配结果
    double final_score = template_best_score;
    int final_digit = template_best_digit;
    
    // 判断是否满足得分差距条件
    bool is_confident = false;
    if (template_scores.size() >= 2 && score_gap > 0.02) {
        is_confident = true;
        ROS_INFO("[模板匹配] 得分差距大于0.02，使用得分最高的模板: 数字%d，得分: %.3f", final_digit, final_score);
    } else if (template_scores.size() == 1) {
        is_confident = true;
        ROS_INFO("[模板匹配] 只有一个模板，直接使用: 数字%d，得分: %.3f", final_digit, final_score);
    } else {
        ROS_WARN("[模板匹配] 得分差距不足0.02，不进行识别");
    }
    ...
    // 显示得分差距信息
    if (template_scores.size() >= 2) {
        putText(display_img, "Score Gap: " + to_string(score_gap).substr(0, 4), Point(10, 100), 
    ...
    if (is_confident && final_digit > 0) {
        status_text = "Status: Confident";
        status_color = Scalar(0, 255, 0); // 绿色表示可信
    } else {
        status_text = "Status: Not Confident";
        status_color = Scalar(0, 0, 255); // 红色表示不可信
    }
    
    putText(display_img, status_text, Point(10, 120), 
            FONT_HERSHEY_SIMPLEX, 0.5, status_color, 1);
    
    // 显示最终识别结果
    string final_result_text = "Final Result: " + to_string(final_digit);
    putText(display_img, final_result_text, Point(10, 140), 
            FONT_HERSHEY_SIMPLEX, 0.5, Scalar(200, 200, 200), 1);
    ...
    while (ok() && current_step != DONE) {
        switch (current_step) {
            ...
            case MOVE_TO_QD_FRONT:
                move_forward(1.0, 0.76); // 优化：前进速度从0.75改为0.76m/s，持续1秒
            ...
            case ROTATE: {
                ...
                // 计算目标总角度（弧度）
                double target_angle = detected_number * 2 * M_PI; // 每圈2π弧度
                
                ROS_INFO("[转圈任务] 角速度: %.2f 弧度/秒, 目标角度: %.2f 弧度 (%.1f圈)", 
                        angular_velocity, target_angle, detected_number);
                ...
                while (current_angle < target_angle) {
                    ...
                    current_angle = angular_velocity * elapsed_time;
                    
                    // 计算当前圈数
                    double current_circles = current_angle / (2 * M_PI);
                    
                    // 每0.5秒显示一次进度
                    static double last_progress_time = 0;
                    if (Time::now().toSec() - last_progress_time > 0.5) {
                        ROS_INFO("[转圈任务] 进度: %.1f圈/%.0f圈 (%.1f%%), 角度: %.2f/%.2f 弧度", 
                    ...
                    // 检查是否接近目标角度，提前减速
                    double remaining_angle = target_angle - current_angle;
                    if (remaining_angle < 0.2) { // 距离目标小于0.2弧度时停止
                        break;
                    }
                    
                    Duration(0.05).sleep(); // 更精确的控制频率
                }
                ...
                // 计算实际转圈数和时间
                double actual_time = Time::now().toSec() - start_time;
                double actual_angle = angular_velocity * actual_time;
                double actual_circles = actual_angle / (2 * M_PI);
                
                ROS_INFO("[转圈任务] 转圈完成，实际转圈数: %.2f圈 (目标: %d圈), 耗时: %.2f秒", 
                        actual_circles, detected_number, actual_time);
                
                // 验证转圈精度
                double error_percent = fabs(actual_circles - detected_number) / detected_number * 100;
                if (error_percent < 5.0) {
                    ROS_INFO("[转圈任务] 转圈精度: 误差 %.1f%%，精度良好", error_percent);
                } else {
                    ROS_WARN("[转圈任务] 转圈精度: 误差 %.1f%%，可能存在偏差", error_percent);
                }
...
// 新增：颜色分类函数实现 - 优化模板匹配
string classify_color(double red, double green, double blue) {
    // 优化颜色分类逻辑，重点匹配模板图片中的人物颜色特征
    
    // 1. 敌方目标颜色特征检测（data/QD.jpg模板）
    // 敌方目标通常为深色服装，优先检测黑色、深灰色等暗色系
    if (red < 50 && green < 50 && blue < 50) {
        return "black"; // 纯黑色 - 敌方特征
    }
    
    // 深灰色检测（敌方常见颜色）
    if (red < 80 && green < 80 && blue < 80 && 
        abs(red - green) < 25 && abs(red - blue) < 25 && abs(green - blue) < 25) {
        if (red < 40) return "dark_gray"; // 深灰色 - 敌方特征
        else return "gray"; // 灰色
    }
    
    // 2. 友方目标颜色特征检测（muban/xn.jpg模板）
    // 友方目标通常为浅色或彩色服装，优先检测白色、浅色系
    if (red > 200 && green > 200 && blue > 200) {
        return "white"; // 纯白色 - 友方特征
    }
    
    // 浅灰色检测（友方常见颜色）
    if (red > 150 && green > 150 && blue > 150 && 
        abs(red - green) < 30 && abs(red - blue) < 30 && abs(green - blue) < 30) {
        return "light_gray"; // 浅灰色 - 友方特征
    }
    
    // 3. 彩色服装检测（放宽阈值，提高识别率）
    
    // 红色检测：敌方可能穿红色服装
    if (red > max(green, blue) + 40 && red > 100) {
        if (red > 150 && green < 100 && blue < 100) return "red"; // 纯红色
        else return "reddish"; // 偏红色
    }
    
    // 蓝色检测：友方可能穿蓝色服装
    if (blue > max(red, green) + 40 && blue > 100) {
        if (blue > 150 && red < 100 && green < 100) return "blue"; // 纯蓝色
        else return "bluish"; // 偏蓝色
    }
    
    // 绿色检测：中性颜色
    if (green > max(red, blue) + 40 && green > 100) {
        if (green > 150 && red < 100 && blue < 100) return "green"; // 纯绿色
        else return "greenish"; // 偏绿色
    }
    
    // 黄色检测：友方可能穿黄色服装
    if (red > 120 && green > 120 && blue < 100) {
        if (red > 150 && green > 150) return "yellow"; // 黄色
        else return "yellowish"; // 偏黄色
    }
    
    // 粉色/桃红色检测：友方可能穿粉色服装
    if (red > 100 && green > 70 && blue < 100) {
        if (red > 120 && abs(red - green) < 50 && red > green + 15) {
            return "pink"; // 粉色/桃红色
        }
    }
    
    // 浅蓝色检测：友方可能穿浅蓝色服装
    if (blue > 120 && red < 100 && green < 120) {
        if (blue > max(red, green) + 30) {
            return "light_blue"; // 浅蓝色 - 友方特征
        }
    }
    
    // 橙色检测：中性颜色
    if (red > 130 && green > 80 && blue < 70) {
        if (red > 150 && green > 90) return "orange"; // 橙色
    }
    
    // 紫色检测：中性颜色
    if (red > 90 && blue > 90 && green < 80) {
        if (red > 110 && blue > 110) return "purple"; // 紫色
    }
    
    // 棕色检测：敌方可能穿棕色服装
    if (red > 80 && green > 60 && blue < 60) {
        if (red > 100 && green > 70) return "brown"; // 棕色
    }
    
    // 4. 基于模板匹配的智能颜色识别
    // 如果颜色接近敌方模板特征（深色系），优先判断为敌方相关颜色
